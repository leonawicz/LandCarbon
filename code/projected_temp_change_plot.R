...
b.cru <- mask(crop(b.cru, shp), shp)
b.cccma <- mask(crop(b.cccma, shp), shp)
b.echam <- mask(crop(b.echam, shp), shp)
cccma.delta <- b.cccma-b.cru
echam.delta <- b.echam-b.cru
d.shp <- fortify(shp)

brick_to_dt <- function(b, model){
    data.table(rasterToPoints(b)) %>% melt(id.vars=c("x", "y"), variable.name="Season", value.name="Temperature") %>% mutate(Model=model)
}

d <- brick_to_dt(cccma.delta, "CCCMA") %>% bind_rows(brick_to_dt(echam.delta, "ECHAM")) %>% data.table
...
rng.all <- range(d$Temperature)

g1a <- ggplot(d, aes(x, y)) + geom_raster(aes(fill=Temperature)) +
    ...
    scale_fill_distiller(palette="Spectral", breaks=seq(rng.all[1], rng.all[2], length=5)) +
    labs(title=expression(atop("Seasonal temperature deltas"~(degree~C), "2090-2099 model mean - 1950-2013 CRU 3.2 mean")))

rng <- group_by(d, Season) %>% summarise(Min=min(Temperature), Max=max(Temperature))
rng2 <- group_by(d, Season, Model) %>% summarise(Min=min(Temperature), Max=max(Temperature)) %>% melt(id.vars=c("Season", "Model"), value.name="Temperature")
get_seq <- function(i) round(seq(rng$Min[i], rng$Max[i], length=6), 1)
get_ylim <- function(i) ylim(rng[i, c(Min, Max)] + c(-1, 1)*0.1*diff(rng[i, c(Min, Max)]))
...
g2a <- ggplot(d %>% filter(Season=="Winter"), aes(x=Model, y=Temperature)) + ggbp + ggpt[[1]] + ylm[[1]] + ggann[[1]] + tp_theme
g2b <- ggplot(d %>% filter(Season=="Spring"), aes(x=Model, y=Temperature)) + ggbp + ggpt[[2]] + ylm[[2]] + ggann[[2]] + tp_theme
g2c <- ggplot(d %>% filter(Season=="Summer"), aes(x=Model, y=Temperature)) + ggbp + ggpt[[3]] + ylm[[3]] + ggann[[3]] + tp_theme
g2d <- ggplot(d %>% filter(Season=="Fall"), aes(x=Model, y=Temperature)) + ggbp + ggpt[[4]] + ylm[[4]] + ggann[[4]] + tp_theme
